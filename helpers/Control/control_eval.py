...
def create_density_umap_for_control_model(model, device, test_dataset, subset_name, collapse_tapped_sequence):
    """
    Generate the umap for the given model and dataset setting.
    Args:
        :param model: The GrooveControl_VAE model to be evaluated
    ...
    intensities = test_dataset.intensities
...
def get_piano_rolls_for_control_model(vae_model, test_dataset, device,
                                            num_examples=8, normalizing_fn=None, reduce_dim=True):
    """
    ...
    for hvo_seq in hvo_sequences:
        ...
        in_grooves = torch.tensor(np.repeat(hvo_seq.flatten_voices(reduce_dim=reduce_dim)[np.newaxis, :], len(density_values), axis=0),
                                  dtype=torch.float32)
...
def get_control_model_density_prediction_averages(model, test_dataset, device, batch_size=64, normalizing_fn=None, reduce_dim=True):

    hvo_seq_set = test_dataset.get_hvo_sequences()
    hvo_sequences = random.sample(hvo_seq_set, batch_size)
    in_grooves = torch.tensor(np.array([hvo_seq.flatten_voices(reduce_dim=reduce_dim) for hvo_seq in hvo_sequences]), dtype=torch.float32)
